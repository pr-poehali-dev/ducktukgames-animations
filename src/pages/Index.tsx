...
const GAMES = [
  {
    ...
    description: "Загадочная утка ждёт твоих кликов. Собирай монеты, открывай тайны и прокачивай способности.",
    emoji: "🦆",
    tag: "КЛИКЕР",
    tagColor: "#FF3CAC",
    accentColor: "#FF3CAC",
  ...
  },
  {
    ...
    description: "Управляй утёнком на супербыстрой машинке. Обгоняй соперников, собирай бонусы!",
    emoji: "🏎️",
    tag: "ГОНКИ",
    tagColor: "#FFD93D",
    accentColor: "#FFD93D",
  ...
  },
  {
    ...
    description: "Утка-паркурщик покоряет городские крыши. Прыгай, уворачивайся, лети над пропастью!",
    emoji: "🦅",
    tag: "ПАРКУР",
    tagColor: "#00F5FF",
    accentColor: "#00F5FF",
  ...
  return (
    <div className="min-h-screen bg-duck-darker grid-bg relative overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute top-[-10%] left-[-5%] w-96 h-96 rounded-full opacity-20 blur-[120px]"
          style={{ background: "radial-gradient(circle, #FF3CAC, transparent)" }} />
        <div className="absolute top-[30%] right-[-5%] w-80 h-80 rounded-full opacity-15 blur-[100px]"
          style={{ background: "radial-gradient(circle, #FFD93D, transparent)" }} />
        <div className="absolute bottom-[10%] left-[30%] w-72 h-72 rounded-full opacity-15 blur-[100px]"
      ...
      <header className="relative z-10 flex items-center justify-between px-6 py-4 border-b border-white/5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="font-russo text-sm text-duck-orange tracking-[0.3em]">GAMES</span>
          </div>
        </div>
      ...
      <div className="md:hidden flex gap-2 px-4 pt-4 relative z-10">
        {(["games", "shop"] as Section[]).map((s) => (
          <button key={s} onClick={() => setActiveSection(s)}
            className={`flex-1 py-2.5 rounded-xl font-russo text-sm transition-all ${
              activeSection === s
          ...
          <div className="animate-slide-up">
            <div className="text-center mb-14">
              ...
              <p className="text-white/50 text-lg max-w-lg mx-auto font-nunito">
                Зарабатывай монеты в играх и трать их в магазине
              </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {GAMES.map((game, i) => (
                <div key={game.id} className="card-game rounded-3xl overflow-hidden group"
                  style={{ animationDelay: `${i * 0.1}s` }}>
                  <div className="relative h-52 overflow-hidden">
                    <img src={game.img} alt={game.title}
                      className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                    <div className="absolute inset-0 bg-gradient-to-t from-duck-darker via-transparent to-transparent opacity-80" />
                    <div className="absolute top-3 left-3">
                      <span className="font-russo text-xs px-3 py-1 rounded-full"
                        style={{ background: `${game.tagColor}22`, color: game.tagColor, border: `1px solid ${game.tagColor}55` }}>
                        {game.tag}
                      </span>
                    </div>
                    ...
                    <div className="absolute bottom-3 right-4 text-4xl animate-float">{game.emoji}</div>
                  </div>
                  <div className="p-5">
                    <h3 className="font-russo text-white text-lg leading-tight mb-1">{game.title}</h3>
                    <p className="text-sm font-bold mb-3" style={{ color: game.accentColor }}>{game.subtitle}</p>
                    <p className="text-white/50 text-sm mb-4 leading-relaxed">{game.description}</p>
                    <div className="flex items-center justify-between">
                      ...
                      <button onClick={() => setActiveGame(game.id)}
                        className="btn-primary px-6 py-2.5 rounded-full text-sm font-russo">
                        ▶ Играть
                      </button>
                    </div>
                  </div>
              ...
              <div className="flex animate-marquee whitespace-nowrap gap-12">
                {[...Array(2)].map((_, ri) => (
                  <span key={ri} className="flex items-center gap-12">
                    {TICKER_ITEMS.map((t) => (
                      <span key={t} className="font-russo text-white/20 text-sm tracking-widest">{t}</span>
                    ))}
                  </span>
                ))}
              </div>
            ...
            <div className="mt-10 grid grid-cols-3 gap-4">
              {[
                ...
                <div key={s.label} className="card-game rounded-2xl p-5 text-center">
                  <Icon name={s.icon as "Users"} size={28} className="mx-auto mb-2" style={{ color: s.color }} />
          ...
          <div className="animate-slide-up">
            <div className="text-center mb-10">
              <div className="inline-flex items-center gap-2 bg-duck-yellow/10 border border-duck-yellow/30 px-4 py-1.5 rounded-full mb-5">
                <span className="text-duck-yellow text-xs">🛒</span>
                <span className="font-russo text-duck-yellow text-xs tracking-widest">МАГАЗИН ПРЕДМЕТОВ</span>
              </div>
              ...
              <p className="text-white/50 font-nunito">
                У тебя <span className="text-duck-yellow font-bold">{coins.toLocaleString()} монет</span> — трать с умом
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {filteredShop.map((item) => {
                ...
                return (
                  <div key={item.id} className="shop-item-card rounded-2xl p-5 relative overflow-hidden">
                    ...
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <span className="text-xs font-russo px-2.5 py-1 rounded-full"
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="font-russo text-lg" style={{ color: item.color }}>
                          {item.price.toLocaleString()}
                        </span>
                      </div>
                      <button onClick={() => handleBuy(item)} disabled={owned || !canAfford}
                        className={`px-4 py-2 rounded-xl font-russo text-sm transition-all ${
                          owned ? "bg-white/10 text-white/40 cursor-not-allowed"
      ...
      {activeGame && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="relative w-full max-w-3xl max-h-[95vh] overflow-y-auto scrollbar-hide">
            {/* Close */}
            <button onClick={() => setActiveGame(null)}
              className="absolute top-0 right-0 z-10 w-10 h-10 rounded-full bg-white/10 border border-white/20 flex items-center justify-center text-white hover:bg-white/20 transition-all">
              <Icon name="X" size={18} />
            </button>
            ...
            {(() => {
              ...
              return g ? (
                <div className="mb-5 flex items-center gap-3 pr-12">
                  ...
                  <div>
                    <h2 className="font-russo text-white text-lg leading-none">{g.title}</h2>
                    <p className="text-xs mt-0.5" style={{ color: g.accentColor }}>{g.subtitle}</p>
                  </div>
                  <div className="ml-auto flex items-center gap-2 bg-duck-yellow/10 border border-duck-yellow/30 px-3 py-1.5 rounded-full">
                    <span>🪙</span>
                    <span className="font-russo text-duck-yellow">{coins.toLocaleString()}</span>
                  </div>
                </div>
              ) : null;
            })()}
            ...
            <div className="flex justify-center">
              {activeGame === "clicker" && (
                <div className="w-full card-game rounded-2xl p-6" style={{ minHeight: 380 }}>
                  <ClickerGame onCoinsEarned={handleCoinsEarned} />
                </div>
              )}
              {activeGame === "racing" && <RacingGame onCoinsEarned={handleCoinsEarned} />}
              {activeGame === "parkour" && <ParkourGame onCoinsEarned={handleCoinsEarned} />}
            </div>
          </div>
        </div>
      )}
